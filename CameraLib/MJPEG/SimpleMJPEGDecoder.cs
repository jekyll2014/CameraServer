...
namespace SimpleMJPEGStreamViewer
{
    static class SimpleMJPEGDecoder
    {
        ...
        /// <returns></returns>
        public async static Task StartAsync(Action<Bitmap> action, string url, string login = null, string password = null, CancellationToken? token = null, int chunkMaxSize = 1024, int frameBufferSize = 1024 * 1024)
        {
            ...
            using (var cli = new HttpClient())
            {
                ...
                using (var stream = await cli.GetStreamAsync(url, tok).ConfigureAwait(false))
                {
        ...
        static void ParseStreamBuffer(Action<Bitmap> action, byte[] frameBuffer, ref int frameIdx, int streamLength, byte[] streamBuffer, ref bool inPicture, ref byte previous, ref byte current)
        {
            var idx = 0;
        ...
        static void ParsePicture(Action<Bitmap> action, byte[] frameBuffer, ref int frameIdx, ref int streamLength, byte[] streamBuffer, ref int idx, ref bool inPicture, ref byte previous, ref byte current)
        {
            do
            {
                previous = current;
                ...
                if (previous == picMarker && current == picEnd)
                {
                    ...
                    using (var s = new MemoryStream(frameBuffer, 0, frameIdx))
                    {
                        try
                        {
                            img = new Bitmap(Image.FromStream(s));
                        }
                        catch
                        {
